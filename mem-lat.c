...
// 代码出自
// https://mp.weixin.qq.com/s/QNgMS0gOXhZml8l_towAbw
...
int main(int argc, char* argv[])
{
    ...
    unsigned long memsize = 0x800000; /* 1/4 LLC(last level cache, L3) size of skylake, 1/5 of broadwell */
    ...
    //
    // | 1:stride | 2:stride | 3:stride | ... | size-1:stride | 0:stride|
    //
    for (i = 0; i < size - 1; i++) {
    ...
    *(char **)&mem[indices[size-1] * stride] = (char*)&mem[indices[0] * stride];
    ...
    printf("Buffer size: %5ld KB, stride %d, time %ld.%06ld s, latency %.2f ns\n", 
